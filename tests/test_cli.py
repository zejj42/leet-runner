...
def test_list_names_every_problem_in_the_repo_and_pages_only_a_tall_list_on_a_terminal(monkeypatch, capsys):
    ...
    assert extras and all(row.lstrip("　 ").startswith(("lc", "x")) for row in extras)                  # off the chart: labelled
...
def test_update_brings_new_problems_and_leaves_what_you_wrote_alone(tmp_path, monkeypatch, capsys):
    ...
    said = capsys.readouterr().out
    assert "Updated  0.0.1 → 0.0.2" in said and "· two" in said and "· Merge Two Sorted Lists     leet read merge-two-sorted-lists" in said or "· 003_merge_two_sorted_lists" in said and again == [True]
